...
for i, case in enumerate(cases):
    for rate in case.reaction_rates:
        ...
        if rate <= 1e-25:
            continue
        log_timescales.append(np.log10(1/rate))
        tiles.append(loc_names[i])

# Create DataFrame
df = pd.DataFrame(dict(log_timescales=log_timescales, tiles=tiles))
...
tiles.map(sns.kdeplot, "log_timescales",
      bw_adjust=.5, clip_on=False,
      fill=True, alpha=1, linewidth=1.5)
tiles.map(sns.kdeplot, "log_timescales", clip_on=False, color="w", lw=2, bw_adjust=.5)
...
tiles.map(label, "log_timescales")
...
tiles.set(xlabel='Reaction Timescale [sec/(molec/cc)]')
